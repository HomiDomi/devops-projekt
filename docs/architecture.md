...
## Zašto kontejneri, ne virtualni strojevi

Sustav se sastoji od pet lagano povezanih servisa (frontend, api, worker,
postgres, redis) koji komuniciraju isključivo preko mreže. Za ovakvu
arhitekturu kontejneri su prikladniji izbor od punih VM-ova iz nekoliko
razloga:

- **Gustoća i brzina pokretanja:** svih pet servisa dijeli isti Linux kernel
  hosta/node-a. Pokretanje ili restart poda traje sekunde (samo se pokreće
  proces unutar postojećeg kernela), dok bi svaki servis u zasebnom VM-u
  nosio pun boot OS-a (desetci sekundi do minuta) i vlastiti kernel overhead
  na svakoj instanci.
- **Iskorištenost resursa:** worker i api su relativno lagani procesi
  (Node.js) koji ne opravdavaju rezervaciju punih virtualnih strojeva s
  vlastitim memory/CPU footprintom OS-a. Kontejneri dijele resurse hosta uz
  `requests`/`limits` granularnost na razini poda, ne cijelog stroja.
- **Skalabilnost:** `api` i `frontend` imaju više replika (rolling
  update/rollback demonstriran upravo na `frontend`). Horizontalno
  skaliranje kontejnera (dodavanje replika) je gotovo trenutno; ekvivalentno
  skaliranje VM-ova zahtijeva puno dulji ciklus provisioninga.
- **Konzistentnost razvoj → produkcija:** isti Containerfile koji se koristi
  za lokalni Podman Compose razvoj producira identičnu sliku koja ide u
  produkciju na OpenShiftu — nema razlike u OS sloju između razvojnog i
  produkcijskog okruženja, što VM pristup (različiti image-ovi/AMI-jevi za
  dev i prod) teže garantira.

Ono što bi VM pristup nudio, a kontejneri ovdje namjerno žrtvuju, jest jača
izolacija na razini kernela (svaki VM ima vlastiti kernel, kontejneri dijele
kernel hosta). U ovom projektu taj rizik je ublažen kroz OpenShiftov
`restricted` SCC (proizvoljni non-root UID, bez privilegiranih kontejnera)
i NetworkPolicy segmentaciju — dovoljna razina izolacije za ovaj kontekst,
uz puno nižu operativnu cijenu od pune VM izolacije.
...
neobrađene narudžbe u tranzitu, ne povijest narudžbi. Ovaj kompromis
opravdava izostanak PVC-a: RDB persistencija je eksplicitno isključena
